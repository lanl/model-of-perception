...
def _read_mesh_as_polydata(mesh_path: str):
    ...
    if mesh_path.lower().endswith(".vtp"):
        ...
        try:
            ...
            raise RuntimeError(
                "vtk is required to read .vtp files. Install with: pip install vtk"
            ) from e
    ...
    if not isinstance(mesh, pv.PolyData) or mesh.n_points == 0:
        ...
        raise RuntimeError(
            f"Unsupported or empty surface mesh from {mesh_path}: {type(mesh)}"
        )
...
def save_aligned_vtp(
    ...
    if template_mesh_path.lower().endswith(".vtp"):
        ...
        try:
            ...
            raise RuntimeError(
                "vtk is required to write .vtp files. Install with: pip install vtk"
            ) from e
    ...
    try:
        ...
        raise RuntimeError(
            "pyvista is required to write .vtp files. Install with: pip install pyvista"
        ) from e
...
def apply_similarity(
    P: np.ndarray, R: np.ndarray, t: np.ndarray, s: float
) -> np.ndarray:
...
def pca_init_candidates(P: np.ndarray, Q: np.ndarray, allow_scale: bool):
    ...
    print(
        "[init] singular values normalized by sqrt(N) to remove point-count dependence."
    )
...
def icp_align(
    ...
    try:
        ...
        raise RuntimeError(
            "scipy is required for ICP (cKDTree). Install with: pip install scipy"
        ) from e
...
def chamfer_distance(A: np.ndarray, B: np.ndarray) -> float:
    ...
    try:
        ...
        raise RuntimeError(
            "scipy is required for Chamfer distance. Install with: pip install scipy"
        ) from e
...
def plot_meshes(
    ...
    for i in range(3):
        ...
        arr_qm = pv.Arrow(
            start=cQ_map, direction=axes_Q_map[:, i], scale=float(len_Q[i])
        )
    ...
    if save_mode:
        ...
        fig2_path = os.path.join(
            save_dir, f"{output_stem}_02_best_alignment_panels.png"
        )
...
def main():
    ...
    ap.add_argument(
        "--vtp",
        required=True,
        help="Path to reference mesh (.vtp/.glb, aligned target)",
    )

    ap.add_argument(
        "--vtp2",
        default=None,
        help="Optional second mesh (.vtp/.glb) to align to the first (skips synthetic transform)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=10.0,
        help="Max abs translation per axis (uniform in [-tmax, tmax])",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=45.0,
        help="Max rotation angle in degrees (applied by slerp from identity)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.0,
        help="Stddev of Gaussian noise added to misaligned points",
    )

    ap.add_argument(
        "--allow_scale",
        action="store_true",
        help="Estimate a uniform scale along with R,t",
    )
    ...
    ap.add_argument(
        "--no_plot", dest="plot", action="store_false", help="Disable plotting (alias)"
    )
    ...
    if args.vtp2:
        ...
        if ang < 1e-12:
            ...
            if axis_norm < 1e-12:
                ...
                R_true = (
                    np.eye(3) + math.sin(theta) * K + (1 - math.cos(theta)) * (K @ K)
                )
    ...
    for i, cand in enumerate(candidates_to_run):
        ...
        Q_aligned_i = apply_similarity(
            Q_mis, R_est_align_i, t_est_align_i, s_est_align_i
        )
    ...
    if args.save_aligned_vtp:
        ...
        saved_path = save_aligned_vtp(
            template_path, Q_aligned, args.save_aligned_vtp, output_stem
        )
