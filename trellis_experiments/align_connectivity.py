...
def _axis_angle_rotation(axis: str, degrees: int) -> np.ndarray:
...
def _right_angle_rotations() -> list[tuple[str, np.ndarray]]:
    """All unique proper rotations generated by 90-degree turns about x/y/z."""
    seen: set[tuple[int, ...]] = set()
    rotations: list[tuple[str, np.ndarray]] = []
    for ax in (0, 90, 180, 270):
        Rx = _axis_angle_rotation("x", ax)
        for ay in (0, 90, 180, 270):
            Ry = _axis_angle_rotation("y", ay)
            for az in (0, 90, 180, 270):
                Rz = _axis_angle_rotation("z", az)
                R = Rz @ Ry @ Rx
                key = tuple(int(v) for v in R.ravel())
                if key in seen:
                    continue
                seen.add(key)
                desc_parts = []
                if ax:
                    desc_parts.append(f"x{ax}")
                if ay:
                    desc_parts.append(f"y{ay}")
                if az:
                    desc_parts.append(f"z{az}")
                desc = "+".join(desc_parts) if desc_parts else "identity"
                rotations.append((desc, R))
    return rotations


def pca_init_candidates(
    P: np.ndarray,
    Q: np.ndarray,
    allow_scale: bool,
    init_mode: str = "both",
    ...
    Builds PCA/SVD sign-flip x permutation initializations plus right-angle
    source rotations and returns them.
    """
    cP = P.mean(axis=0)
    cQ = Q.mean(axis=0)
    Qc = Q - cQ
    ...
    if init_mode in ("rot90", "both"):
        for rot_label, R_rot in _right_angle_rotations():
            R = R_rot
            s = s_base if allow_scale else 1.0
            # Right-angle seeds are defined as pure rotations about the origin.
            # If translation refinement is enabled, Kabsch/ICP can add translation later.
            t = np.zeros(3, dtype=np.float64)
            Q_guess = apply_similarity(Q, R, t, s)
            cost = chamfer_distance(P, Q_guess)
            print(
                f"[init] grid={rot_label} det={np.linalg.det(R):.3f} chamfer={cost:.6g}"
            )
            candidates.append({
                "init_kind": "grid",
                "grid_rot": rot_label,
    ...
    if not candidates:
        raise RuntimeError(f"Unsupported init_mode {init_mode}")
    return candidates
...
def icp_align(
    ...
    for it in range(max_iter):
        ...
        R_delta, t_delta, s_delta = kabsch_svd(
            P_match,
            Q_curr,
...
def kabsch_svd(P: np.ndarray, Q: np.ndarray, allow_scale: bool = False, allow_translation: bool = True):
    """
...
def _candidate_label(cand: dict) -> str:
    if cand.get("init_kind") == "grid":
        return f"grid={cand['grid_rot']}"
    return f"perm={cand.get('perm', (0, 1, 2))} signs={cand.get('signs', (1, 1, 1))}"
...
def plot_meshes(
    ...
    unit_cube = pv.Cube(bounds=(-0.5, 0.5, -0.5, 0.5, -0.5, 0.5))
    ...
    pl0.add_mesh(unit_cube, style="wireframe", color="black", line_width=1.5)
    pl0.add_legend(
        [
            ["P principal axes", p_colors[0]],
            ["Q principal axes", q_colors[0]],
            ["Q axes mapped by best transform", q_map_colors[0]],
            ["unit cube [-0.5,0.5]^3", "black"],
        ],
        bcolor="white",
        face="rectangle",
    )
    ...
    best_tr = min(trials, key=lambda x: x["cd_after"])
    label = f"best {_candidate_label(best_tr)}"
    ...
    pl.add_mesh(unit_cube, style="wireframe", color="black", line_width=1.5)
    pl.add_legend(
        [
            ["reference", "dodgerblue"],
            ["misaligned source", "tomato"],
            ["unit cube [-0.5,0.5]^3", "black"],
        ],
        bcolor="white",
        face="rectangle",
    ...
    pl.add_legend(
        [
            ["reference", "dodgerblue"],
            ["best-run initialization", "goldenrod"],
            ["unit cube [-0.5,0.5]^3", "black"],
        ],
        bcolor="white",
        face="rectangle",
...
def main():
    ...
    ap.add_argument("--allow_translation", dest="allow_translation", action="store_true", help="Estimate/apply translation")
    ap.add_argument("--no_translation", dest="allow_translation", action="store_false", help="Disable translation in initialization and alignment")
    ap.add_argument("--allow_kabsch", dest="allow_kabsch", action="store_true", help="Refine each initial guess with Kabsch/ICP")
    ap.add_argument("--no_kabsch", dest="allow_kabsch", action="store_false", help="Skip Kabsch/ICP and keep the best initial guess")
    ap.add_argument(
        "--init_mode",
        choices=["svd", "rot90", "both"],
        default="both",
        help="Initialization family: PCA/SVD directions, 90-degree rotations, or both",
    )
    ap.add_argument(
        "--icp_mode",
    ...
    ap.set_defaults(plot=False, allow_translation=True, allow_kabsch=True)
    args = ap.parse_args()
    ...
    if args.vtp2:
        ...
        if args.allow_translation:
            ...
            t_true = np.zeros(3, dtype=np.float64)
        s_true = 1.0
    ...
    candidates = pca_init_candidates(
        P,
        Q_mis,
        args.allow_scale,
    ...
    if args.icp_mode == "best_init":
        ...
        print(
            f"\n[icp] init_mode={args.init_mode} "
            f"kabsch={'on' if args.allow_kabsch else 'off'} "
            f"mode=best_init -> running only "
            f"{_candidate_label(candidates_to_run[0])} "
        ...
        print(
            f"\n[icp] init_mode={args.init_mode} "
            f"kabsch={'on' if args.allow_kabsch else 'off'} mode=all -> "
            f"running {len(candidates_to_run)} initializations"
        )
    ...
    for i, cand in enumerate(candidates_to_run):
        ...
        if args.allow_kabsch:
            ...
            R_est_align_i = cand["R0"]
            t_est_align_i = cand["t0"]
            s_est_align_i = cand["s0"]
            Q_init_vis_i = apply_similarity(Q_mis, R_est_align_i, t_est_align_i, s_est_align_i)
            n_iter_i = 0
        ...
        trials.append({
            "idx": i,
            "init_kind": cand.get("init_kind", "pca"),
            "perm": cand.get("perm"),
            "signs": cand.get("signs"),
            "grid_rot": cand.get("grid_rot"),
    ...
    best_label = _candidate_label(best_trial)
    ...
    print(f"best initialization: {best_label}")
    print(f"rotation_angle_est_deg ~ {rotation_angle_deg(R_est_align):.6f}")

    print("\n=== ICP runs executed ===")
    for tr in trials:
        print(
            f"run {tr['idx']}: {_candidate_label(tr)} "
