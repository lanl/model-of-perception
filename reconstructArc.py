...
#    lab = color.lab2lch(color.rgb2lab(rgb_array.reshape(1, -1, 3))).reshape(-1, 3)
    ...
    # compute lab arclength along the colormap curve by just using the euclidean dist of lab values
    deltas = np.linalg.norm(np.diff(lab, axis=0), axis=1)
    arc = np.cumsum(deltas)
    new_num_samples = int(np.ceil(arc[-1]) / 2.9)
...
def print_colormap_arclengths():
    """
    Compute and print arclengths for Rainbow, Spectral, Coolwarm, and Viridis colormaps.
...
if len(sys.argv) > 3:
    ...
    print("Usage: python reconstructArc.py <data_file> <checkpoint_folder> [colormap_name]")
    print("Example: python reconstructArc.py data.vti ./checkpoint Spectral")
    sys.exit(1)

# Print colormap arclengths
